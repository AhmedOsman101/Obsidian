...
var DocumentRenderer = class {
	...
	async loadComponents(view) {
		...
		const loadChildren = async (
			...
			try {
				if (
					((_b = component == null ? void 0 : component.constructor) == null
						? void 0
						: _b.name) === "SheetElement"
				) {
	...
	async untilRendered() {
		while (
			ppIsProcessing ||
			Date.now() - ppLastBlockDate < this.optionRenderSettlingDelay
		) {
	...
	removeFrontMatter(node) {
		node
			.querySelectorAll(".frontmatter, .frontmatter-container")
			.forEach((node2) => node2.remove());
	}
				...
				case 1 /* CONVERT_TO_OBSIDIAN_URI */:
					{
						...
						if (className === "tag") {
							linkNode.href =
								this.vaultSearchUri +
								"&query=tag:" +
								encodeURIComponent(node2.getAttribute("href"));
						} else {
	...
	makeCheckboxesReadOnly(node) {
		node
			...
			.forEach((node2) => node2.setAttribute("disabled", "disabled"));
	}
	removeCollapseIndicators(node) {
		node
			.querySelectorAll(".collapse-indicator")
			.forEach((node2) => node2.remove());
	}
	removeButtons(node) {
	...
	removeAllFootnotes(node) {
		node
			.querySelectorAll("section.footnotes")
		...
		node.querySelectorAll("img").forEach((img) => {
			if (img.src) {
				if (
					...
					this.options.convertSvgToBitmap
				) {
	...
	async replaceImageSource(image) {
		...
		if (imageSourcePath.startsWith(this.vaultLocalUriPrefix)) {
			let path = imageSourcePath
				.substring(this.vaultLocalUriPrefix.length + 1)
				.replace(/[?#].*/, "");
			path = decodeURI(path);
	...
	async readFromVault(path, mimeType) {
		...
		return (
			...
			(0, import_obsidian.arrayBufferToBase64)(data)
		);
	}
...
var _CopyDocumentAsHTMLSettingsTab = class extends import_obsidian.PluginSettingTab {
	...
	display() {
		...
		new import_obsidian.Setting(containerEl)
			...
			.setDesc(
				"If checked, the filename is inserted as a level 1 header. (only if an entire document is copied)",
			)
			.addToggle((toggle) =>
				toggle
		...
		new import_obsidian.Setting(containerEl)
			...
			.setDesc(
				"If checked, only generate a HTML fragment and not a full HTML document. This excludes the header, and effectively disables all styling.",
			)
			.addToggle((toggle) =>
				toggle
		...
		const useCustomStylesheetSetting = new import_obsidian.Setting(containerEl)
			.setName("Provide a custom stylesheet")
			.setDesc(
				"The default stylesheet provides minimalistic theming. You may want to customize it for better looks. Disabling this setting will restore the default stylesheet.",
			);
		const customStylesheetSetting = new import_obsidian.Setting(containerEl)
		...
		useCustomStylesheetSetting.addToggle((toggle) => {
			customStylesheetSetting.settingEl.toggle(
				this.plugin.settings.useCustomStylesheet,
			);
			toggle
		...
		const useCustomHtmlTemplateSetting = new import_obsidian.Setting(
			containerEl,
		)
			.setName("Provide a custom HTML template")
			.setDesc(
				_CopyDocumentAsHTMLSettingsTab.createFragmentWithHTML(`For even more customization, you can 
...
Note that the template is not used if the "Copy HTML fragment only" setting is enabled.`),
		...
		const customHtmlTemplateSetting = new import_obsidian.Setting(containerEl)
			.setDesc(
				_CopyDocumentAsHTMLSettingsTab.createFragmentWithHTML(`
...
<ul>
	...
	<li><code>\${body}</code>: the document body</li>
		...
		useCustomHtmlTemplateSetting.addToggle((toggle) => {
			customHtmlTemplateSetting.settingEl.toggle(
				this.plugin.settings.useCustomHtmlTemplate,
			);
			toggle
...
var CopyDocumentAsHTMLPlugin = class extends import_obsidian.Plugin {
	async onload() {
		...
		this.addCommand({
			...
			checkCallback: this.buildCheckCallback((view) =>
				this.copyFromView(view, view.editor.somethingSelected()),
			),
		});
		this.addCommand({
			id: "copy-as-html",
