...
var L = (i, t) => () => (t || i((t = { exports: {} }).exports, t), t.exports),
	G = (i, t) => {
		for (var e in t) b(i, e, { get: t[e], enumerable: !0 });
	},
	x = (i, t, e, C) => {
		if ((t && typeof t == "object") || typeof t == "function")
			for (const n of U(t))
				!$.call(i, n) &&
					...
					b(i, n, {
						get: () => t[n],
						enumerable: !(C = q(t, n)) || C.enumerable,
					});
		return i;
	};
var J = (i, t, e) => (
		...
		x(
			t || !i || !i.__esModule
				? b(e, "default", { value: i, enumerable: !0 })
				: e,
			i,
		)
	),
	K = (i) => x(b({}, "__esModule", { value: !0 }), i);
var Z = L((_) => {
...
var S = L((r) => {
			...
			(Object.create
				? (i, t, e, C) => {
						...
						(!n ||
							...
							(n = { enumerable: !0, get: () => t[e] }),
							Object.defineProperty(i, C, n);
					}
				: (i, t, e, C) => {
						C === void 0 && (C = e), (i[C] = t[e]);
					}),
		F =
			(r && r.__exportStar) ||
			((i, t) => {
	...
	r.getApiSafe =
		r.getDefer =
		r.isPluginEnabled =
		r.PluginNotEnabledError =
		r.pluginId =
			void 0;
	F(Z(), r);
	F(A(), r);
	F(R(), r);
	r.pluginId = "obsidian-front-matter-title-plugin";
	var m = class extends Error {};
	r.PluginNotEnabledError = m;
	function Y(i) {
		var t, e, C;
		return (C =
			(e =
				(t = i == null ? void 0 : i.plugins) === null || t === void 0
					? void 0
					: t.enabledPlugins) === null || e === void 0
				? void 0
				: e.has(r.pluginId)) !== null && C !== void 0
			? C
			: !1;
	}
	r.isPluginEnabled = Y;
	function B(i) {
		...
		const s =
				(e =
					(t = i == null ? void 0 : i.plugins) === null || t === void 0
						? void 0
						: t.getPlugin(r.pluginId)) !== null && e !== void 0
					? e
					: null,
			h =
				(n =
					(C = s == null ? void 0 : s.getDefer) === null || C === void 0
						? void 0
						: C.call(s)) !== null && n !== void 0
					? n
					: null;
		if (h === null)
			throw new m(`Plugin ${r.pluginId} is not enabled or old version`);
		return h;
	}
	r.getDefer = B;
	function Q(i) {
...
var l = require("obsidian"),
	...
	u = "recent-files",
	E = class extends l.ItemView {
		constructor(e, C, n) {
			super(e);
			this.redraw = () => {
				var o;
				const e = this.app.workspace.getActiveFile(),
					C = createDiv({ cls: "nav-folder mod-root" }),
					n = C.createDiv({ cls: "nav-folder-children" }),
					s = (0, z.getApiSafe)(this.app),
					g =
						s && s.getEnabledFeatures().contains("explorer")
							...
							: null;
				this.data.recentFiles.forEach((a) => {
					var T;
					const d = n.createDiv({
						...
						j = g && (T = g.resolve(a.path)) != null ? T : a.basename;
					f.setText(j),
						(0, l.setTooltip)(d, a.path),
						e && a.path === e.path && p.addClass("is-active"),
						p.setAttr("draggable", "true"),
						p.addEventListener("dragstart", (c) => {
							if (!(a != null && a.path)) return;
							const v = this.app.metadataCache.getFirstLinkpathDest(a.path, ""),
								w = this.app.dragManager,
								D = w.dragFile(c, v);
							w.onDragStart(c, D);
						}),
						p.addEventListener("mouseover", (c) => {
							a != null &&
								a.path &&
						...
						p.addEventListener("contextmenu", (c) => {
							if (!(a != null && a.path)) return;
							const v = new l.Menu();
							v.addItem((D) =>
								D.setSection("action")
									...
									.setIcon("file-plus")
									.onClick(() => {
										this.focusFile(a, "tab");
									}),
							);
							const w = this.app.vault.getAbstractFileByPath(
						...
						p.addEventListener("click", (c) => {
							if (!a) return;
							const v = l.Keymap.isModEvent(c);
							this.focusFile(a, v);
						}),
						f.addEventListener("mousedown", (c) => {
							a &&
								c.button === 1 &&
								(c.preventDefault(), this.focusFile(a, "tab"));
						});
					const k = p.createDiv({
						cls: "recent-files-file-delete menu-item-icon",
					});
					(0, l.setIcon)(k, "lucide-x"),
						k.addEventListener("click", async (c) => {
							c.stopPropagation(), await this.removeFile(a), this.redraw();
						});
				}),
					this.contentEl.setChildrenInPlace([C]);
			};
			...
			this.updateData = async (e) => {
				const C = this.data.recentFiles.length;
				this.data.recentFiles = this.data.recentFiles.filter(
					(s) => s.path !== e.path,
				);
				let n = C !== this.data.recentFiles.length;
				this.plugin.shouldAddFile(e) &&
					(this.data.recentFiles.unshift({
						basename: e.basename,
						path: e.path,
					}),
					(n = !0)),
					n && (await this.plugin.pruneLength());
			};
			this.update = async (e) => {
				await sleep(100), e && (await this.updateData(e), this.redraw());
			};
			this.focusFile = (e, C) => {
				const n = this.app.vault.getFiles().find((s) => s.path === e.path);
				n
					? this.app.workspace.getLeaf(C).openFile(n)
					: (new l.Notice("Cannot find a file with that name"),
						(this.data.recentFiles = this.data.recentFiles.filter(
							(s) => s.path !== e.path,
						)),
	...
	M = class extends l.Plugin {
		constructor() {
			...
			this.pruneOmittedFiles = async () => {
				const e = this.data.recentFiles.length;
				(this.data.recentFiles = this.data.recentFiles.filter(
					this.shouldAddFile,
				)),
			...
			this.pruneLength = async () => {
				const e = this.data.recentFiles.length - (this.data.maxLength || V);
				e > 0 &&
			...
			this.shouldAddFile = (e) => {
				var g, o;
				const C = this.data.omittedPaths.filter((a) => a.length > 0),
					n = (a) => {
						try {
							return new RegExp(a).test(e.path);
						} catch (d) {
							return (
								console.error("Recent Files: Invalid regex pattern: " + a), !1
							);
						}
					};
				if (C.some(n)) return !1;
				const s = this.app.vault.getFileByPath(e.path);
				if (s) {
					const a = this.data.omittedTags.filter((f) => f.length > 0),
						d =
							((o =
								(g = this.app.metadataCache.getFileCache(s)) == null
									? void 0
									: g.frontmatter) == null
								? void 0
								: o.tags) || [],
					...
					if (typeof d == "string") {
						if (p(d)) return !1;
					} else if (d.some(p)) return !1;
				}
				const h = this.app.internalPlugins.getEnabledPluginById("bookmarks");
				return !(
					s &&
			...
			this.handleRename = async (e, C) => {
				const n = this.data.recentFiles.find((s) => s.path === C);
				n &&
					((n.path = e.path),
			...
			this.handleDelete = async (e) => {
				...
				(this.data.recentFiles = this.data.recentFiles.filter(
					...
					C !== this.data.recentFiles.length &&
						(this.view.redraw(), await this.saveData());
			};
		...
		async onload() {
			console.log("Recent Files: Loading plugin v" + this.manifest.version),
				await this.loadData(),
				(0, l.addIcon)("sweep", t1),
				this.registerView(u, (e) => (this.view = new E(e, this, this.data))),
				this.addCommand({
		...
		async loadData() {
			this.data = Object.assign(e1, await super.loadData());
		}
		async saveData() {
	...
	P = class extends l.PluginSettingTab {
		constructor(t, e) {
		...
		display() {
			...
			(C.href =
				...
				(C.text = "MDN - Regular expressions"),
				e.append("RegExp patterns to ignore. One pattern per line. See "),
				e.append(C),
				e.append(" for help."),
				new l.Setting(t)
...
`)),
			...
			n.append("Frontmatter tags patterns to ignore. One pattern per line"),
				new l.Setting(t)
...
`)),
				...
				new l.Setting(t).setName("Omit bookmarked files").addToggle((o) => {
					o.setValue(this.plugin.data.omitBookmarks).onChange((a) => {
						(this.plugin.data.omitBookmarks = a),
							this.plugin.pruneOmittedFiles(),
							this.plugin.view.redraw();
					});
				}),
				new l.Setting(t)
					...
					.addText((o) => {
						...
						o.inputEl.setAttr("type", "number"),
							o.inputEl.setAttr("placeholder", V),
							o
								.setValue(
									((a = this.plugin.data.maxLength) == null
										? void 0
										: a.toString()) || "",
								)
								.onChange((d) => {
									...
									if (!Number.isNaN(p) && p <= 0) {
										new l.Notice("List length must be a positive integer");
										return;
									}
								}),
							(o.inputEl.onblur = (d) => {
								...
								(this.plugin.data.maxLength = f),
									this.plugin.pruneLength(),
									this.plugin.view.redraw();
							});
					});
			const s = t.createEl("div", { cls: "recent-files-donation" }),
			...
			s.appendChild(
				H(
					...
					g.parseFromString(a1, "text/xml").documentElement,
				),
			),
				s.appendChild(
					H(
						"https://www.buymeacoffee.com/tgrosinger",
						g.parseFromString(C1, "text/xml").documentElement,
					),
				);
		}
	},
