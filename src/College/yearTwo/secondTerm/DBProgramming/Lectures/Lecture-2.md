...
> [!WARNING]
> *Do not confuse **DECIMAL** with **FLOAT** or **DOUBLE**. They may all show decimals, but only **DECIMAL** is intended for exact financial-style values.*
...
> [!NOTE]
> *JSON objects use `{}` with key-value pairs, while JSON arrays use `[]` with ordered values. The type stores structured JSON, not arbitrary malformed text.*
...
> [!IMPORTANT]
> *A `DROP` clause in **ALTER TABLE** will not work if the column being dropped is the only column left in the table.*
