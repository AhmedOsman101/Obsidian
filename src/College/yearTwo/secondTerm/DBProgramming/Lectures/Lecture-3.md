...
> [!IMPORTANT]
> _`USE` does not require a semicolon, although adding one does no harm. The exam trap is thinking it must end with `;` to work._
...
> [!WARNING]
> _Creating a table without selecting the correct database first can place work in the wrong schema or fail, so database selection must happen before table creation._
...
> [!IMPORTANT]
> _If the requirement is "empty the table but keep it," use **TRUNCATE TABLE**, not **DROP TABLE**._
...
> [!NOTE]
> _Use **MODIFY** when changing definition only, but use **CHANGE** when the column name itself must also change._
